...
public class RemoveTrainCommand
{
    ...
    public RemoveTrainCommand(ITelegramBotClient client, IMediator mediator, long chatId, User user)
    {
        _client = client;
        _mediator = mediator;
        _chatId = chatId;
    ...
    private async Task<InlineKeyboardMarkup> ActiveTrainMenu(int userId, CancellationToken cancellationToken)
    {
        ...
        {
            var data = $"remove_train:{train.Id}";
